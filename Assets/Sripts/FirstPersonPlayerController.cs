...
[RequireComponent(typeof(CapsuleCollider)), RequireComponent(typeof(Rigidbody)), AddComponentMenu("First Person Player Controller")]
...
public class FirstPersonPlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (enableCameraMovement && !controllerPauseState)
        {
            ...
            {
                mouseYInput = mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.X ? CrossPlatformInputManager.GetAxis(verticalAxisNameLook) : -CrossPlatformInputManager.GetAxis(verticalAxisNameLook);
                mouseXInput = mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.Y ? CrossPlatformInputManager.GetAxis(horizontalAxisNameLook) : -CrossPlatformInputManager.GetAxis(horizontalAxisNameLook);
            }
            else
            {
                mouseXInput = Input.GetAxis(horizontalAxisNameLook) * (mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.Y ? 1 : -1);
            }
    ...
    private void FixedUpdate()
    {
        ...
        float horizontalInput = CrossPlatformInputManager.GetAxis(horizontalAxisName);
        float verticalInput = CrossPlatformInputManager.GetAxis(verticalAxisName);
...
#if UNITY_EDITOR
[CustomEditor(typeof(FirstPersonPlayerController)), InitializeOnLoadAttribute]
public class FirstPersonPlayerController_Editor : Editor
{

    FirstPersonPlayerController t;
    ...
    void OnEnable()
    {
        ...
        t = (FirstPersonPlayerController)target;
        loadedAds = false;
    ...
    public override void OnInspectorGUI()
    {
        ...
        t.cameraInputMethod = (FirstPersonPlayerController.CameraInputMethod)EditorGUILayout.EnumPopup(new GUIContent("Input Method", "Determines the method used to rotate camera. \n\nTraditional uses the mouse on all axes. \nTraditional with constraints uses the mouse on the Y axis only. \nRetro uses Keybinds (left and right movement keys) to rotate the camera along the Y axis."), t.cameraInputMethod);
        if (t.cameraInputMethod == FirstPersonPlayerController.CameraInputMethod.Traditional) { t.verticalRotationRange = EditorGUILayout.Slider(new GUIContent("Vertical Rotation Range", "Determines how much range does the camera have to move vertically."), t.verticalRotationRange, 90, 180); }
        if (t.cameraInputMethod == FirstPersonPlayerController.CameraInputMethod.Traditional || t.cameraInputMethod == FirstPersonPlayerController.CameraInputMethod.TraditionalWithConstraints)
        {
            t.mouseInputInversion = (FirstPersonPlayerController.InvertMouseInput)EditorGUILayout.EnumPopup(new GUIContent("Mouse Input Inversion", "Determines if mouse input should be inverted, and along which axes"), t.mouseInputInversion);
        ...
        EditorGUILayout.Space();
        t.fsmode = (FirstPersonPlayerController.FSMode)EditorGUILayout.EnumPopup(new GUIContent("Footstep Mode", "Determines the method used to trigger footsetps."), t.fsmode);
        EditorGUILayout.Space();

        #region FS Static
        if (t.fsmode == FirstPersonPlayerController.FSMode.Static)
        {
        ...
        {
            t.dynamicFootstep.materialMode = (FirstPersonPlayerController.DynamicFootStep.matMode)EditorGUILayout.EnumPopup(new GUIContent("Material Type", "Determines the type of material will trigger footstep audio."), t.dynamicFootstep.materialMode);
            EditorGUILayout.Space();
            #region Wood Section
            ...
            {
                GUILayout.BeginVertical("box");
                if (t.dynamicFootstep.materialMode == FirstPersonPlayerController.DynamicFootStep.matMode.physicMaterial)
                {
